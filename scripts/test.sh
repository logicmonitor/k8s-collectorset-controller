...
lint_packages() {
  ...
  gometalinter --aggregate --vendor --exclude="zz_generated" --exclude="api" --disable-all --deadline=1200s --enable=deadcode --enable=dupl --enable=errcheck --enable=goconst --enable=gocyclo --enable=gofmt --enable=goimports --enable=golint --enable=gosec --enable=gotypex --enable=ineffassign --enable=interfacer --enable=maligned --enable=metacheck --enable=misspell --enable=nakedret --enable=structcheck --enable=test --enable=testify --enable=unconvert --enable=unparam --enable=varcheck --enable=vet --enable=vetshadow ./...
}
